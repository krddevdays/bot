...
async def delete_messages(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_message_id = context.job.data['help_message_id']
    
    await context.bot.delete_message(chat_id=chat_id, message_id=help_message_id)
    await context.bot.delete_message(chat_id=chat_id, message_id=context.job.data['command_message_id'])

async def command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, *, message_template: str) -> None:
    """Unified command handler."""
    command_message = update.message
    message = await command_message.reply_text(message_template)
...
async def ping_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await command_handler(update=update, context=context, message_template=md("I'm alive, {username}!", user=update.effective_user))
